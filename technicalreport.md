...
<img width="1270" height="700" alt="Image" src="https://github.com/user-attachments/assets/c317c3f2-a307-4e89-8a1f-af2b2c238f84" />
...
**2) Prowler Scan** -After successful multi-cloud provisioning, automated security scanning was performed using Prowler across AWS and Azure environments. Prowler executed 300+ compliance and security checks (including CIS, NIST, PCI-DSS, and GDPR controls) and generated detailed findings in JSON and HTML formats. This step established the baseline security posture and identified misconfigurations, exposed resources, and non-compliant settings for further remediation.

**Video Walkthroughs**
- [Video 6](https://github.com/user-attachments/assets/b04bbe7f-0b11-4f1e-8a2c-d43734b8dd93)

**3) Chroma DB embeddings and OpenAI Remediation Code** - Compliance documents (PCI-DSS, NIST, CIS AWS) were converted into vector embeddings using ChromaDB. These embeddings were stored in a persistent vector database  and with prowler raw findings in form of json file were used as context for OpenAI (GPT-4o-mini) to automatically generate targeted Terraform remediation code for each critical finding. This created an intelligent, context-aware remediation engine capable of producing production-ready IaC fixes.

**Video Walkthroughs**
- [Video 7](https://github.com/user-attachments/assets/feffbdca-232f-4a1f-8144-9dd92547e38a)

**4) OPA Rego Policy Check for OpenAI remediation Code** - Every Terraform remediation code generated by OpenAI was automatically validated against custom OPA Rego policies before acceptance. The policies checks for encryption , network access configuration rules , TLS versions for handshakes , backup management configuration and cloud configuration that could violate NIST,CIS,PCI-DSS compliance benchmarks 

**Video Walkthroughs** 
- [Video 8](https://github.com/user-attachments/assets/56768a86-062b-4480-ac71-2f6350af84d1)

**5) Steampipe Queries of Prowler findings** - Prowler findings (now filtered based on high severity,new severe issue and failed status code) were ingested into Steampipe for SQL-based querying and analysis. Custom queries were written to filter particular data from prowler filtered findings like name,tags and more from cloud resources which could be used later for risk quantification & detailed data visualization using streamlit

**Video Walkthroughs**
- [Video 9](https://github.com/user-attachments/assets/b6b71d18-7f2a-4346-8aa7-5c9e4b826185)

**6) Risk Quantification using IBM data breach report 2025 metrics** - Risk quantification was performed using the FAIR (Factor Analysis of Information Risk) methodology to calculate the Annualized Loss Expectancy (ALE) for every Prowler finding. For each issue, threat event frequency was derived from severity, loss magnitude was determined from asset data classification (Highly Sensitive / Sensitive / Internal), and control effectiveness was mapped using Prowler check results and Steampipe tags (e.g., MFA, encryption, least privilege, public exposure). The engine generated a complete risk report with per-finding ALE values, executive summary statistics (Total ALE, Average ALE, severity distribution), and audit-ready JSON output for compliance and prioritization.

**Video Walkthroughs**
- [Video 10](https://github.com/user-attachments/assets/979fa5c7-21bf-453e-a120-dbc7d7baf22e)

**7) Compliance Report Generation** - An auditor-grade compliance report was automatically generated as a professional PDF evidence package suitable for SOC2 Type II, ISO 27001, and PCI-DSS audits. The report incorporates FAIR-based ALE calculations, risk heat maps, top-10 prioritized risks, detailed control mappings (NIST 800-53, SOC2, ISO27001), remediation roadmap with phased timelines, asset inventory summary, control effectiveness tables, and full appendices with assumptions and limitations. Both detailed PDF and machine-readable JSON versions were produced for compliance teams and external auditors.

**Video Walkthroughs**
- [Video 11](https://github.com/user-attachments/assets/c32ff130-a5fe-4e6d-af4a-6a6a277b6c61)

**8) Streamlit dashboard for data visualization & analytics** - A Streamlit dashboard was developed to provide real-time, interactive visibility into the entire GRC pipeline. The interface displays risk heat maps, ALE trends, severity distribution, compliance score coverage & Data sensitivity charts. Users can filter by cloud provider, service, or severity, drill down into individual findings, and export the full PDF/JSON report with one click

**Video Walkthroughs**
- [Video 11](https://github.com/user-attachments/assets/2f20aeb7-4b4e-4194-ad7e-c52ed94cbace)





### 2.3 Multi-Cloud Data Flow Architecture
...
| AWS IAM          | Azure Service Principal | Cross-cloud auth   | Azure Key Vault for data encryption for storage account      |
